...
export class TariffsController {
  ...
  async calculatePrice(
    ...
    const tariffResult = await this.tariffService.getTariffByPhonePrefix(body.phone);
  ...
  async checkOperator(
    @Body() body: { phone: string },
  ): Promise<SingleResponse<{ operator: string; price: number; phone_ext: string }>> {
    const tariffResult = await this.tariffService.getTariffByPhonePrefix(body.phone);
  ...
  async bulkCalculatePrice(
    ...
    for (const phone of body.phones) {
      try {
        const tariffResult = await this.tariffService.getTariffByPhonePrefix(phone);
