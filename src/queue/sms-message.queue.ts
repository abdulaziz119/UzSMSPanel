...
export class SmsMessageQueue {
  ...
  async handleSendToGroup(
    ...
    try {
      ...
      // Process large groups in batches to avoid memory issues
      const BATCH_SIZE = 100; // Process 100 SMS at a time

      if (items.length > BATCH_SIZE) {
        this.logger.log(
          `Processing large group with ${items.length} contacts in batches of ${BATCH_SIZE}`,
        );
